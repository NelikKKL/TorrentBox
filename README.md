...
написания (версия 8.x). `winreg` (реестр Windows) и `egui::ViewportCommand`
(фокус окна) — тоже сверены по актуальной документации. Если после
`cargo build` компилятор всё же укажет на несовпадение — почти наверняка
